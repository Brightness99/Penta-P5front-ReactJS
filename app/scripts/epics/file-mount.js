...
export function fileMountFetch(action$) {
  return action$.ofType(FileMountConstants.FILE_MOUNT_FETCH_REQUEST)
    .switchMap(() => {
      const endpoint = '/v2/montagem-do-arquivo';
...
export function fileMountItemFetch(action$) {
  return action$.ofType(FileMountConstants.FILE_MOUNT_ITEM_FETCH_REQUEST)
    .switchMap(action => {
      const endpoint = `/v2/montagem-do-arquivo/${action.payload.slug}`;
      ...
      return rxAjax({
        ...
        .map(data => {
          if (data.status === 200 && data.response) {
            return {
              type: FileMountConstants.FILE_MOUNT_ITEM_FETCH_SUCCESS,
              payload: data.response,
              meta: { updatedAt: getUnixtime() },
            };
          }
          ...
          return {
            type: FileMountConstants.FILE_MOUNT_ITEM_FETCH_FAILURE,
            payload: { message: 'Algo de errado não está correto' },
            meta: { updatedAt: getUnixtime() },
          };
        })
        .takeUntil(action$.ofType(AppConstants.CANCEL_FETCH))
        .defaultIfEmpty({ type: FileMountConstants.FILE_MOUNT_ITEM_FETCH_CANCEL })
        .catch(error => {
          if (error.status === 404) {
            push('/404');
          }
